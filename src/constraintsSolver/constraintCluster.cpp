...
constraintCluster::constraintCluster(std::vector<std::shared_ptr<constraint_abstract>> constraints, std::vector<variable_ptr> vars, int verbose):
...
int constraintCluster::solve_DL(double eps)
{
    int n_vars = mVariables.size();		// Number of variables in the system
    int n_cons = mConstraints.size();	// Number of equations in the system
    Eigen::VectorXd X(n_vars), h_sd(n_vars), h_gn(n_vars), h_dl(n_vars), X_new(n_vars), 	// Variables values, attempt change in variables and candidate variables values
                    e(n_cons), e_new(n_cons), g(n_vars); 	// Error over variables, candidate variables and error scaled with jacobian  
