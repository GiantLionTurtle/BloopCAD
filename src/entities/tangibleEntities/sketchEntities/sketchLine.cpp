...
{
	return { { { mPointA->pos_var()->x }, { mPointA->pos_var()->x->expr() } },
			 { { mPointB->pos_var()->x }, { mPointB->pos_var()->x->expr() } }};
}
std::vector<subEquationsSystem> sketchLine::horizontality()
{
	return { { { mPointA->pos_var()->y }, { mPointA->pos_var()->y->expr() } },
			 { { mPointB->pos_var()->y }, { mPointB->pos_var()->y->expr() } }};
}